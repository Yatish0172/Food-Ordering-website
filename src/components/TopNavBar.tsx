...
interface Props { currentScreen: ScreenType; onNavigate: (screen: ScreenType) => void; cartCount: number; onOpenCart: () => void; onOpenAbout: () => void; searchQuery: string; onSearchChange: (q: string) => void; currentUser: User | null; }

export const TopNavBar = ({ currentScreen, onNavigate, cartCount, onOpenCart, onOpenAbout, searchQuery, onSearchChange, currentUser }: Props) => {
  if (currentScreen === 'checkout') return <header className="fixed top-0 z-50 w-full px-4 md:px-16 py-4 flex justify-between items-center glass-panel border-b border-white/10"><button onClick={()=>onNavigate('home')} className="font-['Bricolage_Grotesque'] text-xl md:text-3xl font-bold text-[#ffb2ba]">The Karaoke Kitchen</button><button onClick={()=>onNavigate('menu')} className="font-['Space_Mono'] text-sm font-bold text-[#00dbe9]">✕ CANCEL</button></header>;
  return (
