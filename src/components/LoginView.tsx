...
export const LoginView = ({ currentUser, onAuthenticated, onLogout, onNavigate }: { currentUser: User | null; onAuthenticated: (user: User) => void; onLogout: () => void; onNavigate: (screen: ScreenType) => void }) => {
  ...
  const chooseMode = (nextMode: 'login' | 'register') => { setMode(nextMode); setError(''); };
  ...
  return (
    <main className="pt-28 pb-16 px-4 min-h-screen">
      <section className="max-w-md mx-auto glass-panel border border-white/10 rounded-3xl p-7 md:p-8">
        ...
        <div className="grid grid-cols-2 bg-[#0c1322] rounded-full p-1 my-6"><button type="button" onClick={() => chooseMode('login')} className={'rounded-full py-2 text-xs font-bold ' + (mode==='login'?'bg-[#00dbe9] text-[#002022]':'')}>SIGN IN</button><button type="button" onClick={() => chooseMode('register')} className={'rounded-full py-2 text-xs font-bold ' + (mode==='register'?'bg-[#ffb2ba] text-[#670020]':'')}>REGISTER</button></div>
        <form onSubmit={submit} className="space-y-4">
          {mode === 'register' && <><div className="grid grid-cols-2 gap-3"><input required minLength={2} maxLength={60} autoComplete="given-name" aria-label="First name" className={field} placeholder="First name" value={form.firstName} onChange={update('firstName')} /><input required maxLength={60} autoComplete="family-name" aria-label="Last name" className={field} placeholder="Last name" value={form.lastName} onChange={update('lastName')} /></div><input required type="tel" inputMode="tel" autoComplete="tel" aria-label="Phone number" pattern="[+0-9 -]{10,16}" className={field} placeholder="Phone number" value={form.phone} onChange={update('phone')} /></>}
          <input required type="email" autoComplete="email" maxLength={120} aria-label="Email address" className={field} placeholder="Email address" value={form.email} onChange={update('email')} />
          <input required type="password" aria-label="Password" autoComplete={mode==='login'?'current-password':'new-password'} minLength={mode==='register'?12:1} maxLength={72} className={field} placeholder={mode==='register'?'Password · minimum 12 characters':'Password'} value={form.password} onChange={update('password')} />
          {mode === 'register' && <><input maxLength={180} autoComplete="street-address" aria-label="Street address" className={field} placeholder="Street address (optional)" value={form.streetAddress} onChange={update('streetAddress')} /><div className="grid grid-cols-2 gap-3"><input maxLength={100} aria-label="Apartment or landmark" className={field} placeholder="Landmark" value={form.aptSuite} onChange={update('aptSuite')} /><input inputMode="numeric" autoComplete="postal-code" aria-label="PIN code" pattern="[0-9]{6}" className={field} placeholder="PIN code" value={form.zipCode} onChange={update('zipCode')} /></div></>}
